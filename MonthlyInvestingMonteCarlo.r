...
# Make a data frame for the simulations
...
# Monte Carlo simulation of returns for 20% volatility
volatility <- 0.2
...
# Monte Carlo simulation of returns for 30% volatility
volatility <- 0.3
for (i in 1:n_simulations){
	for (j in 1:I(tradingdays * length_years - 1)){
		# If month start, then invest more by monthy_reinvestment
...
ggplot(d, aes(x = Return)) +
	...
	# Plot the average returns for each volatility
