...
# Make data frames y and z for the simulation results x keeps the simulations temporarily and is
# replaced after every simulation
x <- as.data.frame(matrix(nrow = tradingdays * length_years, ncol = 1))
...
# Plot the results
ggplot(d, aes(x = Return)) + geom_histogram(aes(y = ..count.., fill = Volatility)) +
